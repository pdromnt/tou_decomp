/*
 * sound.cpp - Recovered sound selection, sample table, and music lifecycle
                              ...
                              const char *filename)
{
    ...
    AudioSampleHandle handle = Audio_LoadSample(path, bLoop != 0);
 ...
 * The original binary has 16 "complex" sample loads
...
void FUN_0040e130(void)
{
    ...
    Audio_PlayMusic(path, musVol, DAT_004877a4 == 0x97);
}

/* ===== Pause_Audio_Streams (0040E7E0) - Actually UNPAUSES ===== */
void Pause_Audio_Streams(void)
{
    if (g_SoundEnabled && Audio_HasMusic())
        Audio_PauseMusic(0);
}
