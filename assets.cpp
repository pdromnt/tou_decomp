...
/* Supplemental UTF-8/Latin-1 glyph order generated by
 * tools/generate_latin_fonts.py. Original atlas pixels remain untouched. */
static const unsigned char Font_Latin_Map[] = {
    0xC0,0xC1,0xC2,0xC3,0xC7,0xC9,0xCA,0xCD,0xD1,0xD3,0xD4,0xD5,0xDA,0xDC,
    0xE0,0xE1,0xE2,0xE3,0xE7,0xE9,0xEA,0xED,0xF1,0xF3,0xF4,0xF5,0xFA,0xFC,
    0xA1,0xBF
};
...
static void Parse_Font_Data(int font_idx, unsigned char *img_data, int w, int h,
                            const unsigned char *char_map, int char_count,
                            bool initialize)
{
    int glyph_index = 0;
    bool in_char = false;
...
void Load_Fonts(void)
{
    ...
    for (int f = 0; f < 4; f++) {
        ...
        Parse_Font_Data(f, data, w, h, Font_Char_Map, 95, true);
        stbi_image_free(data);

        char supplement_path[64];
        snprintf(supplement_path, sizeof(supplement_path), "data/%s_latin.tga",
                 f == 0 ? "f_large" : f == 1 ? "f_mini" :
                 f == 2 ? "f_med" : "f_tiny5d");
        data = stbi_load(supplement_path, &w, &h, &c, 3);
        if (data) {
            Parse_Font_Data(f, data, w, h, Font_Latin_Map,
                            (int)sizeof(Font_Latin_Map), false);
            stbi_image_free(data);
        } else {
            LOG("[FONT] Missing optional supplement %s\n", supplement_path);
        }
        loaded++;
    }
