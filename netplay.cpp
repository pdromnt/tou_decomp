...
const uint16_t kProtocolVersion = 3;
                  ...
                  const uint8_t *payload, size_t payload_size)
{
    const size_t limit = type == MESSAGE_INITIAL_SNAPSHOT
        ? static_cast<size_t>(kMaximumSnapshotPayload) + 4u : kMaximumPayload;
    ...
    if (message_size > kMaximumQueuedBytes ||
        ...
        Disconnect(connection, "outgoing network queue exceeded safety limit");
        return;
    }
...
void Flush(Connection *connection)
{
    ...
    if (result > 0) {
        ...
        Disconnect(connection, "send failed");
    }
}
...
bool IsMessageShapeValid(uint16_t type, size_t size)
{
    ...
    if (type == MESSAGE_PING || type == MESSAGE_PONG)
        return size == 0;
    if (g_mode == MODE_HOST) {
        return (type == MESSAGE_HELLO && size == 10) ||
    ...
    if (g_mode == MODE_CLIENT) {
        return (type == MESSAGE_WELCOME && size == 1) ||
               ...
               (type == MESSAGE_REJECT && size > 0 && size < 128) ||
...
void NormalizeHostLobbySlots(void)
{
    uint8_t next_slot = 1;
    for (size_t i = 0; i < g_clients.size(); ++i) {
                   ...
                   const uint8_t *payload, size_t size)
{
    if (g_mode == MODE_HOST && type == MESSAGE_HELLO && !connection->hello) {
        if (payload[0] >= 9 || payload[1] >= 2) {
            Disconnect(connection, "invalid player profile");
            return;
        }
        if (Get64(payload + 2) != LevelCatalogFingerprint()) {
            static const char reason[] = "Level catalog mismatch; use the same game package";
            QueueMessage(connection, MESSAGE_REJECT,
                         reinterpret_cast<const uint8_t *>(reason), sizeof(reason) - 1);
            Flush(connection);
            Disconnect(connection, "client level catalog mismatch");
            return;
        }
        ...
        if (connection->slot == 0xff) {
            Disconnect(connection, "LAN session is full");
            return;
        }
    ...
    if (g_mode == MODE_CLIENT && type == MESSAGE_WELCOME) {
        if (payload[0] == 0 || payload[0] >= 4) {
            Disconnect(connection, "invalid player slot");
            return;
        }
    ...
    if (g_mode == MODE_CLIENT && type == MESSAGE_REJECT && size > 0 && size < 128) {
        char reason[128];
        memcpy(reason, payload, size);
        reason[size] = 0;
    ...
    if (g_mode == MODE_CLIENT && type == MESSAGE_START &&
        ...
        if (g_local_slot >= g_player_count || g_player_count < 2 || g_player_count > 4) {
            Disconnect(connection, "invalid player assignment");
            return;
        }
    ...
    if (g_mode == MODE_HOST && type == MESSAGE_CHECKSUM && size == 12) {
        ...
        if (g_host_checksum_ticks[index] == tick && g_host_checksums[index] != remote) {
            SetStatus(Text_Get("lan.status.desync_format"), tick,
                      static_cast<unsigned long long>(g_host_checksums[index]),
    ...
    if (g_mode == MODE_HOST && type == MESSAGE_LEVEL_READY &&
        ...
        if (generation == 0 || generation > g_level_generation + 1) {
            Disconnect(connection, "invalid level synchronization generation");
            return;
        }
    ...
    if (g_mode == MODE_CLIENT && type == MESSAGE_INITIAL_SNAPSHOT &&
        ...
        if (generation != g_level_generation) {
            Disconnect(connection, "host synchronized the wrong level generation");
            return;
        }
        ResetTickTransportState();
        SeedInputDelay();
        if (!SimulationState_Restore(payload + 4, size - 4)) {
            Disconnect(connection, "host initial state did not match the loaded level");
            return;
        }
    ...
    if (type == MESSAGE_DISCONNECT) {
        Disconnect(connection, "peer closed session");
        return;
    }
    ...
    for (;;) {
        ...
        if (result > 0) {
            ...
            if (connection->received.size() > receive_limit - static_cast<size_t>(result)) {
                Disconnect(connection, "incoming network queue exceeded safety limit");
                return;
            }
            connection->received.insert(connection->received.end(), chunk, chunk + result);
            connection->last_received_at = Platform_GetTicks();
        } else if (result == 0) {
            Disconnect(connection, "peer closed connection");
            return;
        } else {
            if (!WouldBlock())
                Disconnect(connection, "receive failed");
            break;
        }
    }
    ...
    while (connection->received.size() - consumed >= 16) {
        ...
        const uint32_t payload_limit =
            (g_mode == MODE_CLIENT && type == MESSAGE_INITIAL_SNAPSHOT)
                ? kMaximumSnapshotPayload + 4u : kMaximumPayload;
...
void PollClientConnect(void)
{
    ...
    if (ready <= 0) {
        if (Platform_GetTicks() - g_server.connect_started > 10000u)
            Disconnect(&g_server, "connection timed out");
        return;
    }
    int error = 0;
#ifdef _WIN32
    ...
    if (getsockopt(g_server.socket, SOL_SOCKET, SO_ERROR,
        ...
        Disconnect(&g_server, "connection failed");
        return;
    }
...
void PollHeartbeat(Connection *connection)
{
    ...
    if (now - connection->last_received_at > 10000u) {
        Disconnect(connection, "peer timed out");
        return;
    }
...
void PollHostAccept(void)
{
    ...
    for (;;) {
        ...
        if (g_clients.size() >= 3) {
            ...
            static const char reason[] = "LAN session is full";
            QueueMessage(&rejected, MESSAGE_REJECT,
                         reinterpret_cast<const uint8_t *>(reason), sizeof(reason) - 1);
            Flush(&rejected);
            CloseSocket(accepted);
            continue;
...
void Netplay_FormatRoster(char *out, size_t out_size)
{
    ...
    for (size_t i = 0; i < g_clients.size(); ++i) {
        ...
        if (used >= out_size)
            break;
        snprintf(out + used, out_size - used, " | P%u T%u",
                 static_cast<unsigned>(client.slot + 1),
                 static_cast<unsigned>(client.team + 1));
    }
}
